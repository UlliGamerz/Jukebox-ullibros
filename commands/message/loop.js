...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!mode || !validModes.includes(mode)) {
            const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> Please specify a valid loop mode!\n**Options:** `off`, `track`, `queue`\nExample: `!loop track`');
        ...
        try {
            ...
            const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> An error occurred while setting loop mode!');
            return message.reply({ embeds: [embed] })
                .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
        }
    }
