...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!position || position < 1) {
            const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> Please provide a valid position number! Example: `!remove 3`');
        ...
        try {
            ...
            const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> An error occurred while removing the song!');
            return message.reply({ embeds: [embed] })
                .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
        }
    }
