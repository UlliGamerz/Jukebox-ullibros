...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        try {
            ...
            const embed = new EmbedBuilder().setDescription('<a:stop:1420040669333946418> Music stopped and disconnected from voice channel!');
            return message.reply({ embeds: [embed] })
                .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));

        } catch (error) {
            console.error('Stop command error:', error);
            const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> An error occurred while stopping music!');
            return message.reply({ embeds: [embed] })
                .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
        }
    }
