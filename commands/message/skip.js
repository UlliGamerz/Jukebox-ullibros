...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        try {
            ...
            const embed = new EmbedBuilder().setDescription(`<a:skip:1420038188533284864> Skipped: **${currentTrack}**`);
            return message.reply({ embeds: [embed] })
                .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));

        } catch (error) {
            console.error('Skip command error:', error);
            const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> An error occurred while skipping the song!');
            return message.reply({ embeds: [embed] })
                .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
        }
    }
