    ...
    async execute(message, args, client) {
        ...
        try {
            ...
            if (!conditions.hasActivePlayer || conditions.queueLength === 0) {
                const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> Queue is empty, nothing to shuffle!');
            ...
            const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> An error occurred while shuffling the queue!');
            return message.reply({ embeds: [embed] })
                .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
        }
    }
