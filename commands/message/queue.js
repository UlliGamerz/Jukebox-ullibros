    ...
    async execute(message, args, client) {
        ...
        try {
            ...
            if (!currentTrack && queue.size === 0) {
                const embed = new EmbedBuilder().setDescription('<a:library:1420038090579247184> Queue is empty!');
                return message.reply({ embeds: [embed] })
                    .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
            }
            ...
            if (queue.size > 0) {
                ...
                if (queueTracks.length > 0) {
                    description += `<a:library:1420038090579247184> **Up Next (${queue.size} songs)**\n`;
            ...
            const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> An error occurred while fetching the queue!');
            return message.reply({ embeds: [embed] })
                .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
        }
    }
