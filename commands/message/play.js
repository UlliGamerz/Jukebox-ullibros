...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        if (!query) {
            const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> Please provide a song to play!');
        ...
        try {
            ...
            if (result.type === 'track') {
                const embed = new EmbedBuilder().setDescription(`<a:tick:1420038198989684736> Added to queue: **${result.track.info.title}**`);
                return message.reply({ embeds: [embed] })
                    .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
            } else if (result.type === 'playlist') {
                const embed = new EmbedBuilder().setDescription(`<a:tick:1420038198989684736> Added **${result.tracks}** songs from playlist: **${result.name}**`);
                return message.reply({ embeds: [embed] })
                    .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
            } else {
                const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> No results found for your query!');
                return message.reply({ embeds: [embed] })
                    .then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
            }

        } catch (error) {
            const embed = new EmbedBuilder().setDescription('<a:cross:1420038015123722250> An error occurred while trying to play music!');
