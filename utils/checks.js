...
class ConditionChecker {
    ...
    getErrorMessage(conditions, action = 'play') {
        if (!conditions.userInVoice) {
            return '<a:cross:1420038015123722250> You need to be in a voice channel to use music commands!';
        }

        if (!conditions.canJoinVoice) {
            return '<a:cross:1420038015123722250> I don\'t have permission to join your voice channel!';
        }
        ...
        if (conditions.hasActivePlayer && !conditions.sameVoiceChannel) {
            ...
            if (conditions.botInCentralVC && !conditions.fromCentral) {
                if (conditions.centralEnabled && conditions.centralVC) {
                    return `<a:cross:1420038015123722250> I'm currently in the central music system! Join <#${conditions.centralVC}> or use the central channel to control music.`;
                }
            }
